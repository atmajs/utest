...
var _tests = [], _index = -1, _listeners = {}, _testsDone;
var RESERVED = [
    '$teardown',
    '$config',
    '$before',
...
function runCase(ctx, fn, done, teardown, key, utest) {
    ...
    function tryWait_Deferred(dfr) {
        ...
        if (atma_utils_1.is_Function(result.catch)) {
            result.catch(onFail);
        }
        return true;
    }
...
exports.watch = function (base, resources, callback) {
    ...
    resources.forEach(function (url) {
        ...
        if (file.exists()) {
            vars_1.io.watcher.watch(file.uri.toLocalFile(), function () {
                var path = file.uri.toLocalFile();
                vars_1.io.File.clearCache(path);
            ...
            if (/socket\.io/i.test(path))
                return;
            vars_1.logger.warn('<utest: watcher> File 404 - ', path);
        }
    });
};
//= private
...
var Runner = /** @class */ (function (_super) {
    ...
    Runner.prototype.notifyTest = function (url) {
        ...
        vars_1.logger.log('Test: ', ('bold<cyan<' + name + '>>').color);
    };
    ...
    Runner.prototype.onFailure = function (data) {
        ...
        if ('actual' in data || 'expected' in data) {
            ...
            if (typeof expect === 'string'
                ...
                vars_1.logger.log(msg.color, data.actual, data.expected);
            }
        }
        if (data.message && data.generatedMessage !== true)
            vars_1.logger.log(' bold<red<::>> '.color + data.message.yellow);
...
var RunnerSuite = /** @class */ (function () {
    ...
    RunnerSuite.prototype.onComplete = function () {
        ...
        var self = this;
        cfg_1.watch(this.base, this.getResources(), function (path) {
            logger_1.log_clearStd();
            vars_1.logger.log(Date.format(new Date, 'HH:mm:ss').bg_cyan
                + ' modified ' + (str_1.color(templateObject_3 || (templateObject_3 = __makeTemplateObject(["bold<", ">"], ["bold<", ">"])), path)));
            self.runTests();
        });
        vars_1.logger.log(str_1.color(templateObject_4 || (templateObject_4 = __makeTemplateObject(["yellow< - watcher active.>"], ["yellow< - watcher active.>"]))));
    };
...
(function () {
    vars_1.global.sinon = require('sinon');
}());
(function () {
    vars_1.global.assert = require('assertion');
}());
(function () {
    vars_1.global.DomTest = require('domtest');
}());
