		...
		(function () {
			...
			var Before = {
				loadBenchmarkLibrary: function loadBenchmarkLibrary(model, next) {
					...
					include.js("/.reference/atma_toolkit/node_modules/utest/node_modules/benchmark/benchmark.js").done(function (resp) {
			...
			function _benchmarkDelegate(model, name, fns) {
				return function (done) {
					...
					suite.on("start", function () {
						return logger.log("Benchmark started '" + name.bold + "'");
					}).on("cycle", function (event) {
						return logger.log(event.target.toString());
					}).on("teardown", model.$teardown).on("complete", function (event) {
	
						Array.from(suite.sort(function (a, b) {
							a = a.stats;b = b.stats;
							return a.mean + a.moe > b.mean + b.moe ? 1 : -1;
						})).map(function (x) {
							return x.toString();
						}).forEach(function (x, i) {
							return logger.log((i + 1 + ". ").bold.green + x);
						});
			...
			resource.inject(pckg).done(function (resp) {
				return setTimeout(function () {
					Object.keys(resp).filter(function (key) {
						...
						logger.log("<utest:Environment> Loaded dependency has no exports `%s`.".yellow.bold, key).log("yellow<Should it be global variables, to bold<dismiss> this error use smth like this:>".color).log("`{ env: [ \"someLib.js::globalVarName\" ] }`".bold);
					});
					callback(resp);
				});
			});
