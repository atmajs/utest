...
exports.sinon = sinon;
var assertion = global.assert = require('assertion');
exports.assertion = assertion;
var domtest = global.DomTest = require('domtest');
...
exports.UTestVars = {
    ...
    remove: function (utest) {
        var i = this.tests.indexOf(utest);
    ...
    function tryWait_Deferred(dfr) {
        ...
        function onSuccess() {
            ...
            if (args.length === 1 && args[0] instanceof UTest) {
                var utest_1 = args[0];
                UTestVars_1.UTestVars.remove(utest_1);
                utest_1.run(function () {
                    // Process self
                    onSuccess();
                });
                return;
        ...
        if (x.added || x.removed) {
            if (/^[ ]*$/.test(val)) {
                ...
                val = val
                    ...
                    return color_1.Color.bg_cyan("[".concat(code, "]"));
                })
                    .join('');
            }
        }
...
function utest_resolveFiles(config) {
    ...
    if ((0, atma_utils_1.is_Array)(config)) {
        for (var i = 0, x = void 0, imax = config.length; i < imax; i++) {
            x = config[i];
    ...
    base = new atma_utils_1.class_Uri(base);
    for (var i = 0, x = void 0, imax = scripts.length; i < imax; i++) {
...
var Runner = /** @class */ (function (_super) {
    ...
    function Runner(config, settings) {
        var _this = this;
        var _a;
        _this = _super.call(this) || this;
        _this.isAction = false;
        _this.isAction = (_a = settings === null || settings === void 0 ? void 0 : settings.isAction) !== null && _a !== void 0 ? _a : false;
        _this.config = config;
        _this.status = exports.status_blank;
        _this.files = utest_resolveFiles(config);
        _this.suites = (0, atma_utils_1.is_Array)(config) ? config : [config];
    ...
    Runner.prototype.notifyTest = function (url) {
        ...
        vars_1.logger.log((0, str_1.color)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["", ": bold<cyan<", ">>"], ["", ": bold<cyan<", ">>"])), this.isAction ? "Tasks" : "Test", name));
    ...
    Runner.prototype.onComplete = function (stats) {
        ...
        if (this.isAction !== true) {
            vars_1.logger
                .log((0, str_1.color)(templateObject_2 || (templateObject_2 = __makeTemplateObject(["\n\nbold<", "<Done.>>"], ["\\n\\nbold<", "<Done.>>"])), failed ? 'red' : 'green'))
                .log((0, str_1.color)(templateObject_3 || (templateObject_3 = __makeTemplateObject(["bold<Assertions>: bold<green<", ">>(bold<red<", ">>)"], ["bold<Assertions>: bold<green<", ">>(bold<red<", ">>)"])), Math.max(total - failed, 0), failed))
                .log((0, str_1.color)(templateObject_4 || (templateObject_4 = __makeTemplateObject(["bold<Timeouts>: bold<", "<", ">>"], ["bold<Timeouts>: bold<", "<", ">>"])), timeouts ? 'red' : 'green', timeouts))
                .log((0, str_1.color)(templateObject_5 || (templateObject_5 = __makeTemplateObject(["bold<Failed Callbacks>: bold<green<", ">>"], ["bold<Failed Callbacks>: bold<green<", ">>"])), callbacks));
        }
        else {
            var time = Date.now() - this.startedAt;
            vars_1.logger
                .log((0, str_1.color)(templateObject_6 || (templateObject_6 = __makeTemplateObject(["\n\nbold<", "<Completed> in ", "ms>"], ["\\n\\nbold<", "<Completed> in ", "ms>"])), failed ? 'red' : 'green', time));
        }
        this.failed = failed;
        this.stats = stats;
    ...
    Runner.prototype.getResources = function () {
        ...
        var resources = this.stats.resources
            ...
            || ((_b = this.getResources) === null || _b === void 0 ? void 0 : _b.call(this))
            || [];
        return resources;
    };
    // assertion events
    Runner.prototype.onFailure = function (data) {
        ...
        if ('actual' in data || 'expected' in data) {
            ...
            if (typeof expect === 'string'
                ...
                vars_1.logger.log((0, str_1.color)(templateObject_7 || (templateObject_7 = __makeTemplateObject(["", ""], ["", ""])), msg), data.actual, data.expected);
            }
        }
        if (data.message && data.generatedMessage !== true)
            vars_1.logger.log((0, str_1.color)(templateObject_8 || (templateObject_8 = __makeTemplateObject([" bold<red<::>> "], [" bold<red<::>> "]))) + data.message.yellow);
        if (data.file && data.line != null) {
            var path = data
                ...
                .replace(/\?.+/, '')
                .replace(/^\//, '');
            var uri = new atma_utils_1.class_Uri(path).isRelative()
                ? new atma_utils_1.class_Uri(base).combine(path).toString()
                : path;
            if (vars_1.io.File.exists(uri)) {
                var source = vars_1.io.File.read(uri, { skipHooks: true });
                var lines = source.split(/\r\n|\r|\n/g);
                var line = lines[data.line - 1];
                var code = line && line.trim();
                ;
                vars_1.logger
                    .log((0, str_1.color)(templateObject_9 || (templateObject_9 = __makeTemplateObject(["  bold<", ">"], ["  bold<", ">"])), path))
                    .log((0, str_1.color)(templateObject_10 || (templateObject_10 = __makeTemplateObject(["  bold<cyan< \u2192 >> bold<", " |> bold<red< ", " >>"], ["  bold<cyan< \u2192 >> bold<", " |> bold<red< ", " >>"])), data.line + 1, code))
                    .log('');
            }
            return;
        }
    ...
    Runner.prototype.onSuccess = function () {
        process.stdout.write((0, str_1.color)(templateObject_11 || (templateObject_11 = __makeTemplateObject(["bold<green< |>>"], ["bold<green< |>>"]))));
    };
    return Runner;
...
function io_connect(config) {
    ...
    var port = config.port || 5777, url = "http://localhost:".concat(port, "/node"), socket = client.connect(url, {
        reconnection: true,
...
function slave_openBrowser(callback) {
    var url = "http://".concat(HOST, ":").concat(PORT, "/utest/");
...
var RunnerClient = /** @class */ (function (_super) {
    ...
    function RunnerClient(config, settings) {
        var _this = _super.call(this, config, settings) || this;
    ...
    RunnerClient.prototype.run = function (done) {
        ...
        (0, io_1.io_connect)(this.config)
            ...
            if (_this.startServer === true) {
                var msg = "Test server connection error - http://localhost:".concat(port, "/utest");
                done(msg);
                return;
            }
            _this.startServer = true;
            vars_1.logger.log((0, str_1.color)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["<utest:server not started> bold<green<starting...>>"], ["<utest:server not started> bold<green<starting...>>"]))));
            (0, slave_1.slave_start)(function (error) {
                if (error) {
            ...
            vars_1.logger.log((0, str_1.color)(templateObject_2 || (templateObject_2 = __makeTemplateObject(["bold<green<server connected>>"], ["bold<green<server connected>>"]))));
            _this.socket = socket;
            socket
                ...
                var message = (0, str_1.color)(templateObject_3 || (templateObject_3 = __makeTemplateObject(["bold<", " ", ">"], ["bold<", " ", ">"])), stats.userAgent.browser.name, stats.userAgent.browser.version);
                vars_1.logger
                    .log(message)
                    .log('');
            })
    ...
    RunnerClient.prototype.runTests = function () {
        vars_1.logger.log((0, str_1.color)(templateObject_4 || (templateObject_4 = __makeTemplateObject(["bold<-->--bold<-->--bold<-->--bold<--> yellow<client>"], ["bold<-->--bold<-->--bold<-->--bold<--> yellow<client>"]))), (0, str_1.color)(templateObject_5 || (templateObject_5 = __makeTemplateObject(["bold<", ">"], ["bold<", ">"])), Date.format(new Date(), 'HH:mm:ss')), '\n');
...
var RunnerFork = /** @class */ (function (_super) {
    ...
    RunnerFork.prototype.run = function (done) {
        ...
        vars_1.logger
            .log('\t\t\t\t>'.bg_yellow)
            .log('Fork'.bg_yellow.black, (0, str_1.color)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["bold<", ">"], ["bold<", ">"])), atma_utils_1.class_Uri.combine(base, cfgPath)))
            .log('\t\t\t\t>'.bg_yellow)
            .log('');
...
var RunnerNode = /** @class */ (function (_super) {
    ...
    function RunnerNode(config, settings) {
        var _this = _super.call(this, config, settings) || this;
    ...
    resource.inject(pckg).done(function (resp) {
        setTimeout(function () {
            Object
                ...
                var injected = eval("typeof ".concat(key, " !== 'undefined' && ").concat(key));
                if (injected) {
                    global[key] = injected;
                    return;
                }
...
function suite_next(callback) {
    ...
    if (((_a = _suite.$config) === null || _a === void 0 ? void 0 : _a.includejs) != null) {
        (_b = global.include) === null || _b === void 0 ? void 0 : _b.cfg(_suite.$config.includejs);
    }
...
var RunnerSuite = /** @class */ (function () {
    function RunnerSuite(configs, settings) {
        ...
        this.isAction = (_a = settings === null || settings === void 0 ? void 0 : settings.isAction) !== null && _a !== void 0 ? _a : false;
    ...
    RunnerSuite.prototype.onComplete = function () {
        ...
        if (this.watch !== true) {
            ...
            if (this.isAction !== true) {
                vars_1.logger
                    .log('')
                    .log(failed === 0
                    ? (0, str_1.color)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["bold<green<Success ", ">>"], ["bold<green<Success ", ">>"])), succeeded) : (0, str_1.color)(templateObject_2 || (templateObject_2 = __makeTemplateObject(["bold<red<Failed ", "/", "/>>"], ["bold<red<Failed ", "/", "/>>"])), failed, succeeded));
            }
            this.callback(failed);
            return;
        }
        closeSocketsOnExit(this);
        var resources = (0, cfg_1.watch)(this.base, this.getResources(), function (path) {
            var dateStr = Date.format(new Date, 'HH:mm:ss').bg_cyan;
            var message = "#".concat(++_this.counter, "  ").concat(dateStr, "  modified ").concat((0, str_1.color)(templateObject_3 || (templateObject_3 = __makeTemplateObject(["bold<", ">"], ["bold<", ">"])), path));
            if (_this.isIdle() === false) {
                vars_1.logger.log(message);
                vars_1.logger.log((0, str_1.color)(templateObject_4 || (templateObject_4 = __makeTemplateObject(["... but the RunnerSuite is red<busy>"], ["... but the RunnerSuite is red<busy>"]))));
                return;
            }
        ...
        vars_1.logger.log((0, str_1.color)(templateObject_5 || (templateObject_5 = __makeTemplateObject(["yellow< - watcher active. ", " Files>"], ["yellow< - watcher active. ", " Files>"])), resources.length));
    };
...
var UTest_1 = _src_UTest;
var UAction = /** @class */ (function (_super) {
    __extends(UAction, _super);
    function UAction(mix, suite, parent) {
        var $suite = typeof mix === 'string' ? suite : mix;
        ...
        return _super.call(this, mix, suite, parent) || this;
    }
    UAction.spawn = function (command) {
    };
    UAction.getCurrent = function () {
    };
    return UAction;
}(UTest_1.UTest));
...
var Action = /** @class */ (function () {
    ...
    Action.prototype.spawn = function () {
        throw new Error('Not Implemented');
        //process_toggle()
    };
    Action.prototype.getLogs = function () {
        throw new Error('Not Implemented');
    };
    return Action;
}());
...
var AtmaAction = {
    ...
    process: function (setts, done) {
        ...
        node_vars_1.NodeVars.suite = new Suite_1.RunnerSuite(node_vars_1.NodeVars.configs, __assign(__assign({}, setts), { isAction: (setts === null || setts === void 0 ? void 0 : setts.action) !== 'test' }));
...
if ((_a = vars_1.global.atma) === null || _a === void 0 ? void 0 : _a.utest_action) {
    exports.AtmaAction = AtmaAction = (_b = vars_1.global.atma) === null || _b === void 0 ? void 0 : _b.utest_action;
}
else {
    ((_c = vars_1.global.atma) !== null && _c !== void 0 ? _c : {}).utest_action = AtmaAction;
}
;
